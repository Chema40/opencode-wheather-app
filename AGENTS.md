...
- Current runtime entrypoint is `src/index.ts`; it implements the weather CLI menu, city storage, geocoding, current weather, and settings.
- `README.md` describes the original weather CLI goal; the implementation now lives under `src/`.
- `package.json` has `build`, `start`, and `dev` scripts; `bun run build` compiles a native `./weather` binary from `src/index.ts`.
- Use Bun directly for setup/run work; install deps with `bun install`.
- Run the CLI with `bun src/index.ts` or `bun run start`.
...
- Source code is organized under `src/actions`, `src/api`, `src/presentation`, `src/storage`, `src/types`, and `src/utils`.
